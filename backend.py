...
# ---------------------------------------------------------------------------
#   TWO-STEP ANSWER FUNCTION (Option 1B)
#   1) LLM returns a raw SQL query (strip out code fences)
#   2) We run the query, feed the result back for summarization
# ---------------------------------------------------------------------------
def answer_user_question(question: str):
    ...
    # FIRST PROMPT: Possibly return an SQL query if relevant
    prompt = f"""
    You are an expert data analyst.
    Given the following database schema:
    {schema}
    And given the user's question: "{question}"
    If the question can be answered by generating an SQL query, output ONLY the SQL query.
    Otherwise, provide a direct natural language answer.
    """
    response = client.models.generate_content(model="gemini-2.0-flash", contents=prompt)
    answer_raw = response.text.strip()

    # STRIP OUT CODE FENCES (```sql ... ```) to avoid confusion
    answer_stripped = re.sub(r"```[a-zA-Z]*", "", answer_raw).replace("```", "").strip()

    # Check if the answer looks like an SQL query
    # We'll do a simpler check: see if it *contains* SELECT
    if "SELECT" in answer_stripped.upper():
        try:
            ...
            # SECOND PROMPT: Summarize the query result
            prompt2 = f"""
            You are an expert data analyst.
            ...
            Please provide a clear, concise, natural language summary of the information in this data,
            and include any recommendations if appropriate. Do NOT include any SQL code in your response.
            """
            response2 = client.models.generate_content(model="gemini-2.0-flash", contents=prompt2)
            final_answer = response2.text.strip()

            # If summarization is empty or still looks like SQL, fallback
        ...
        # If LLM didn't return an SQL query, just pass the natural language answer along
        return answer_raw, None
...
# # --- New Feature: Ask a Question via LLM and SQL ---
# def answer_user_question(question: str):
...
#     soft_skills_count INTEGER.
#     """
#     # First prompt: ask the LLM to decide if an SQL query is appropriate.
#     prompt = f"""
#     You are an expert data analyst.
#     Given the following database schema:
#     {schema}
...
#             # Prepare a second prompt to summarize the SQL query result.
#             prompt2 = f"""
#             You are an expert data analyst.
#             The following JSON array represents the result of an SQL query executed on a database:
#             {df.to_json(orient='records')}
...
#             response2 = client.models.generate_content(model="gemini-2.0-flash", contents=prompt2)
#             final_answer = response2.text.strip()
#             # Fallback if the summarization is empty or still looks like an SQL query.
#             if not final_answer or final_answer.upper().startswith("SELECT"):
#                 final_answer = "I'm sorry, I could not generate a proper summary from the query result."
#             return final_answer, df
#         except Exception as e:
#             return f"Error executing SQL query: {e}", None
#     else:
#         # If the LLM did not output an SQL query, return the direct answer.
#         return answer, None
